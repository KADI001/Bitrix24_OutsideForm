...
import com.example.application.bitrix24.properties.*;
import com.example.application.bitrix24.lists.BList;
...
public class Bitrix24 {
    ...
    public Process getProcess(String blockID, String[] additionalFields){
        Process res = null;
        String params = "IBLOCK_TYPE_ID=" + typeIDStringMap.get(IBlockTypeID.Bitrix_Processes) + "&IBLOCK_ID=" + blockID;
    ...
    public String addProcessElement(String blockID, String elementCode, Map<String, Object> fields){
        ...
        for(Field field: processFields){
            ...
            if(field.getFieldID().contains("PROPERTY_")){
                ...
                if(property.getType() == PropertyType.File){
                    FileProperty fileProperty = (FileProperty) property;

                    Set<String> keys = fields.keySet();

                    for(String key: keys){
                        if(fileProperty.getCode().equals(key)){

                            String[] value = (String[]) fields.get(key);

                            params.append("&FIELDS[" + field.getFieldID() + "]=" + value[0]);
                            params.append("&FIELDS[" + field.getFieldID() + "]=" + "data:image/png;base64," + value[1]);
                        }
                    }
                }
                ...
                if(property.getType() == PropertyType.EList){
                    ...
                    if(isInt == false || eListProperty.getCode().equals("GOD_PROEKTA")){
                ...
                if(property.getType() != PropertyType.List && property.getType() != PropertyType.EList && property.getType() != PropertyType.File){
    ...
    public ListElement getListElementByName(String blockTypeID, String blockID, String elementName){
        ListElement res = null;

        String params = "IBLOCK_TYPE_ID=" + blockTypeID + "&IBLOCK_ID=" + blockID + "&FILTER[NAME]=" + elementName;

        String response = callMethod("lists.element.get", params);
        ...
        try{
            ...
            String iBlockID = element.get("IBLOCK_ID").toString();
            String createdBy = element.get("CREATED_BY").toString();

            res.setName(name);
            res.setId(id);
            res.setiBlockID(iBlockID);
